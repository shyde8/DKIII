...
public class MovingPlatform : MonoBehaviour
{
    ...
    void Update()
    {
        if(gameObject.GetComponent<LeverControlledPlatform>()==null)
        {
        ...
        {
            if(gameObject.GetComponent<LeverControlledPlatform>().isActive==true)
            {
                if(_trackPercentX < .97f)
                {
                    _trackPercentX += _directionX * speed * Time.deltaTime;
                    float x = (positions[1].x - positions[0].x) * _trackPercentX + positions[0].x;
                    transform.position = new Vector3(x, positions[0].y, positions[0].z);
                }                
            }
            else
            {
